    ...
    loop {
        ...
        match command.as_str() {
            ...
            "TURN" | "BEGIN" => {
                ...
                if is_size_20 {
                    let eval_20 = eval_20.as_mut().unwrap();
                    if command.as_str() == "TURN" {
                        let (x, y) = coord_x_y.unwrap();
                        eval_20.add(Coord::from(x, y)).unwrap();
                    }
                    let timeout = info.timeout_turn((&eval_20).as_ref());
                    eval_20.set_turn_timeout(timeout);
                    let coord = eval_20.place_next();
                    if let Some((x, y)) = coord.get() {
                        println!("{x},{y}");
                    }
                } else {
                    let eval_15 = eval_15.as_mut().unwrap();
                    if command.as_str() == "TURN" {
                        let (x, y) = coord_x_y.unwrap();
                        eval_15.add(Coord::from(x, y)).unwrap();
                    }
                    let timeout = info.timeout_turn((&eval_15).as_ref());
                    eval_15.set_turn_timeout(timeout);
                    let coord = eval_15.place_next();
                    if let Some((x, y)) = coord.get() {
                        println!("{x},{y}");
                    }
                }
            ...
            "BOARD" => {
                if is_size_20 {
                    ...
                    loop {
                        ...
                        let coord = Coord::<20>::from(nums[0], nums[1]);
                        if nums[2] == 1 {
                    ...
                    for (&b, &w) in coords_b.iter().zip(coords_w.iter()) {
                        eval_20.add(b).unwrap();
                        eval_20.add(w).unwrap();
                    }
                    if coords_b.len() > coords_w.len() {
                        for &b in &coords_b[coords_w.len()..] {
                            eval_20.add(b).unwrap();
                            eval_20.add(Coord::new()).unwrap(); // pass
                        }
                        eval_20.undo().unwrap();
                    ...
                    // copied from above, size changed to 15
                    let mut coords_own = Vec::new();
                    let mut coords_opp = Vec::new();
                    loop {
                        let mut input = String::new();
                        ...
                        let coord = Coord::<15>::from(nums[0], nums[1]);
                        if nums[2] == 1 {
                            coords_own.push(coord);
                        } else {
                            // 2
                    ...
                    let eval_15 = eval_15.as_mut().unwrap();
                    for (&b, &w) in coords_b.iter().zip(coords_w.iter()) {
                        eval_15.add(b).unwrap();
                        eval_15.add(w).unwrap();
                    }
                    if coords_b.len() > coords_w.len() {
                        for &b in &coords_b[coords_w.len()..] {
                            eval_15.add(b).unwrap();
                            eval_15.add(Coord::new()).unwrap(); // pass
                        }
                        eval_15.undo().unwrap();
                    } else if coords_w.len() > coords_b.len() {
                        for &w in &coords_w[coords_b.len()..] {
